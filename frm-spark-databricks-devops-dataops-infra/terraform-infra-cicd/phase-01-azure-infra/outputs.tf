...
# ------------------------------------------------------------------------------
# Metastore ID (NEW - Critical for Phase 02)
# ------------------------------------------------------------------------------

output "metastore_id" {
  description = "ID of the Unity Catalog Metastore created in this phase"
...
output "pipeline_summary" {
  description = "Summary of Phase 01 deployment"
  value       = <<-EOT

    =============================================================
    ✅ Phase 01 (Azure Infrastructure) Complete!
    =============================================================

    Resources created:
    - Metastore ID:    ${databricks_metastore.this.id}
    - Resource Groups: ${azurerm_resource_group.dev.name}, ${azurerm_resource_group.prod.name}
    - Workspaces:      ${azurerm_databricks_workspace.dev.name}, ${azurerm_databricks_workspace.prod.name}
    - Storage:         ${azurerm_storage_account.dev.name}, ${azurerm_storage_account.prod.name}

    =============================================================
    🚀 Next Steps (CI/CD Pipeline)
    =============================================================
    
    The pipeline will now proceed to Phase 02 (Unity Catalog).
    It will automatically read the outputs above from the remote state.
    
    Check the next job in GitHub Actions for details.
    =============================================================
  EOT
}