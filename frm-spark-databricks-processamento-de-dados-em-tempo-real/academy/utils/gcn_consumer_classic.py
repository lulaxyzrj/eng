...
# Connect as a consumer (client "cursoStreaming")
# Warning: don't share the client secret with others.
consumer = Consumer(config,
                    client_id='7g350302l7baagq9hjtjrms9ru',
                    client_secret='172eugi1r6ta5vu5ijh09qm2fpofedji4es6qjerkm64v08kbt83')

# topics = ['gcn.classic.text.SWIFT_POINTDIR']

# Subscribe to topics and receive alerts
topics = (['gcn.classic.text.AGILE_GRB_GROUND',
...
while True:
    for message in consumer.consume(timeout=1):
        ...
        message_data = {
            'ingestion_blob_time': datetime.now().isoformat(),  # Adicionado para capturar a hora da ingestão
            'topic': message.topic(),
        ...
        directory = '/Volumes/landing/gcn/gcn/arquivos/'
        os.makedirs(directory, exist_ok=True)

        # Save the message to a file in the landing area
        with open(f'{directory}{message.topic()}_{message.offset()}.json', 'w') as file:
            file.write(json.dumps(message_data))

# COMMAND ----------

# MAGIC %md
# MAGIC ### Modelo de código para rodar local:
# MAGIC https://github.com/douglas-engenheirodedados/streamingSupernovasAndBlackholes/blob/main/src/consumidor_gcn_classic.py